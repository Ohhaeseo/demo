...
public class MemberController {
    ...
    public String checkMembers(@ModelAttribute AddMemberRequest request, Model model, HttpServletRequest request2, HttpServletResponse response) {
        try {
            ...
            session.setAttribute("userId_" + sessionId, sessionId); // 사용자별 고유 세션 ID 저장
            session.setAttribute("email_" + sessionId, member.getEmail()); // 사용자별 이메일 저장
            session.setAttribute("name_" + sessionId, member.getName()); // 사용자별 이름 저장
